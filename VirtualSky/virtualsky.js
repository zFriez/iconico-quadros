...
(function (S) {
			...
			for (o = 0; o < ob.length; o++) {
				...
				if (ok || ob[o].src != "lookup") {
					S(document).ajax(ob[o].url, {
						dataType: ob[o].type, "this": this, success: function (data) {
	...
	VirtualSky.prototype.positionCredit = function () {
		this.container.find('.' + this.id + '_credit').css({ position: 'none', top: (parseFloat(this.tall) - this.padding - this.fontsize()) + 'px', left: this.padding + 'px' });
	};
